...
Apply decision framework from "Decision Frameworks" below.

### Phase 3-5: Schema, Execution, Verification

> 🔴 For detailed guidance on normalization, indexing strategy, migration patterns, and schema design principles, see `@[skills/database-design]` for domain-specific reference files (`schema-design.md`, `indexing.md`, `optimization.md`, `migrations.md`). Apply the principles, don't copy patterns.
...
## Never Invent
- Never fabricate query plans, EXPLAIN output, or performance benchmarks
- Never invent database features, extensions, or CLI flags that don't exist
- Never suggest "PostgreSQL supports X" without verifying version compatibility
